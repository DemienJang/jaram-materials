...
교육자료나 워크북을 만든 이유와 활용 흐름이 결과물 안에만 남지 않도록 콘텐츠별 제작노트를 기본 산출물로 추가한다.

### 수정 내용

- 콘텐츠 폴더 표준에 `06_edu_material_blog.md`를 추가했다.
- 교육자료 또는 워크북 제작 시 제작노트를 자동으로 작성하도록 기본 작업 순서에 반영했다.
- 제작노트는 독백체로 작성하고 자료의 목적·질문·활용 흐름을 간결하게 설명하도록 기준을 추가했다.

### 관련 콘텐츠

- `contents/006-dopamine-wanting-liking/06_edu_material_blog.md`

### 다음 점검 사항

- 다음 교육자료·워크북 제작부터 `06_edu_material_blog.md`가 함께 생성되었는지 확인한다.

### 수정 대상

- `guides/03_CARDNEWS_GUIDE.md`
- `tools/export-cardnews.mjs`

### 수정 의도

카드뉴스 제작 후 인스타그램 게시 문구가 누락되지 않도록 기본 산출물에 포함하고, 템플릿 클래스가 바뀌어도 로컬 PNG 렌더링이 계속 작동하도록 내보내기 도구를 보완한다.

### 수정 내용

- 모든 `03_cardnews.md`에 인스타그램 게시 문구 3안을 포함하도록 규칙을 추가했다.
- 경험 중심형, 지식·개념 중심형, 질문·참여 유도형의 세 가지 결을 기본으로 제시했다.
- `tools/export-cardnews.mjs`가 최신 `card-item/card-shell` 구조와 기존 `item/shell` 구조를 모두 인식하도록 수정했다.
- 카드뉴스 PNG 렌더링 기준과 보조 이미지 위치를 가이드에 추가했다.

### 관련 콘텐츠

- `contents/007-conversation-needs/`

### 다음 점검 사항

- 새 카드뉴스 제작 시 `03_cardnews.md`에 게시 문구 3안이 있는지 확인한다.
- HTML 수정 후 `node tools/export-cardnews.mjs <cardnews-folder>`로 PNG를 재생성한다.
...
교육자료와 워크북을 생성할 때 대표 인덱스뿐 아니라 대표 로그도 누락 없이 갱신하도록 운영 규칙을 명확히 한다.

### 수정 내용

- 교육자료·워크북 생성 시 `outputs/materials/index.html`과 `outputs/materials/LOG.md`를 같은 작업 흐름에서 갱신하도록 명시했다.
